...
cacheSolve <- function(x, ...) {
    ...
    data <- x$get()
    # If condition below decides if there is a need to convert non-square matrix into a 
    # possible square matrix based on  two conditions :
    # condition 1 - if nrows = ncols of matrix
    # condition 2 - if  reminder of length of matrix divided by squareroot(length)
    # is zero.
